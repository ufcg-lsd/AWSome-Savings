...
class TestAWSModel(unittest.TestCase):
    ...
    def test_savings_plan_not_integer(self):
        ...
        subprocess.run('python3 ./implementations/py_sp_only/build_simulation.py ./tests/test_data/on_demand_config.csv ./tests/test_data/savings_plan_config.csv ./tests/test_data/total_demand.csv', shell=True, stdout=subprocess.DEVNULL, stderr=subprocess.STDOUT)
    ...
    stderr=subprocess.STDOUT)
        ...
        try:
            ...
            result_cost = pd.read_csv('result_cost.csv')
            actual_cost = result_cost.loc[0, 'total_cost']
            self.assertEqual(actual_cost, 29.4)
        except FileNotFoundError:
            self.fail("The file result_cost.csv was not created.")
