...
nohup /optimizer/build/opt --solve "$proto_path" "$output_directory/on_demand_config.csv" "$output_directory/savings_plan_config.csv" "$output_directory/total_demand.csv" "$logs_directory" > "$logs_directory/output.log" 2> "$logs_directory/error.log"
opt_exit_code=$?
